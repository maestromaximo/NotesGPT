...
def main():
    ...
    root.title("RemarkableGPT")

    # Create a frame to hold the buttons
    main_frame = tk.Frame(root)
    main_frame.pack(fill="both", expand=True)

    # Create the settings and user buttons
    settings_button = tk.Button(root, text="Settings", command=lambda: switch_window(main_frame, settings_window))
    settings_button.place(x=0, y=675, width=100, height=25)

    user_button = tk.Button(root, text="User", command=lambda: switch_window(main_frame, user_window))
    user_button.place(x=400, y=0, width=100, height=25)

    # Create a StringVar to hold the loading message
    loading_message = tk.StringVar()
    loading_message.set("Loading")

    # Create a label to display the loading message
    loading_label = tk.Label(main_frame, textvariable=loading_message)
    loading_label.place(x=200, y=300, width=100, height=50)

    # Use a separate thread to perform the loading tasks
    threading.Thread(target=loading_tasks, args=(root, main_frame, loading_message)).start()

    # Start the event loop
    root.mainloop()

def loading_tasks(root, frame, loading_message):
    # Simulate a long-running task
    root.after(0, loading_message.set, "Loading\n Initializing")
    creds = get_credentials()
    service = build('drive', 'v3', credentials=creds)
    ...
    for course in courses:
        file_path = os.path.join(course.folder, course.main_note.name)
        root.after(0, loading_message.set, f"Loading\n Checking lessons on {course.name}")
        newLesson =checkIfNewLesson(course.main_note, file_path)
        root.after(0, loading_message.set, "Loading.\n Updating lessons")
        updateLessons(course)
    ...
    root.after(0, switch_window, frame, main_view)

def switch_window(frame, window_func):
    # Destroy all widgets from frame
    for widget in frame.winfo_children():
        widget.destroy()

    # Call the function to create new widgets
    window_func(frame)

    # Create the back button in the root window
    back_button = tk.Button(frame.winfo_toplevel(), text="Back", command=lambda: switch_window(frame, main_view))
    back_button.place(x=0, y=675, width=100, height=25)

def main_view(frame):
    # Create the buttons
    load_pdf_button = tk.Button(frame, text="Load PDF", command=lambda: switch_window(frame, load_pdf_window))
    load_pdf_button.place(x=200, y=175, width=100, height=50)

    query_button = tk.Button(frame, text="Query", command=lambda: switch_window(frame, query_window))
    query_button.place(x=200, y=250, width=100, height=50)

    review_button = tk.Button(frame, text="Review", command=lambda: switch_window(frame, review_window))
    review_button.place(x=200, y=325, width=100, height=50)

    check_db_button = tk.Button(frame, text="Check DB", command=lambda: switch_window(frame, check_db_window))
    check_db_button.place(x=200, y=400, width=100, height=50)

def settings_window(frame):
    pass

def user_window(frame):
    pass

def load_pdf_window(frame):
    # Create a StringVar to hold the file path and the status message
    file_path = tk.StringVar()
    status_message = tk.StringVar()

    # Create a button that lets you choose a file
    choose_file_button = tk.Button(frame, text="Choose file", command=lambda: choose_file(file_path, status_message))
    choose_file_button.place(x=200, y=175, width=100, height=50)

    # Create a label to display the file path
    file_path_label = tk.Label(frame, textvariable=file_path)
    file_path_label.place(x=200, y=250, width=100, height=50)

    # Create a label to display the status message
    status_label = tk.Label(frame, textvariable=status_message)
    status_label.place(x=200, y=325, width=100, height=50)

def choose_file(file_path, status_message):
    # Open a file dialog and update the file_path StringVar with the chosen file
    file_path.set(filedialog.askopenfilename())

    # Update the status message
    status_message.set("Loading pdf, please wait a couple minutes")

    # Use a separate thread to run the read_pdf_and_chunk function
    threading.Thread(target=read_pdf_and_chunk, args=(file_path.get(), 200, status_message)).start()

def query_window(frame):
    pass

def review_window(frame):
    pass

def check_db_window(frame):
    pass
